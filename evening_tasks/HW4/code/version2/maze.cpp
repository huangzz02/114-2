    ...
    for (int i = 0; i < rows_; ++i) {
        for (int j = 0; j < cols_; ++j) {
            if (originalMaze_[i][j] == PLAYER) {
                player_ = {i, j};
            } else if (originalMaze_[i][j] == BOX) {
                box_ = {i, j};
            }
        }
    }
    isGameOver_ = false;
    while (!playerPositionHistory_.empty()) playerPositionHistory_.pop(); // clear position history
    while (!boxPositionHistory_.empty()) boxPositionHistory_.pop(); // clear position history
    while (!boxIsMovedHistory_.empty()) boxIsMovedHistory_.pop(); // clear position history
}
