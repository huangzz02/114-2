...
void apply_card_effect(const Card& card, int& pile, bool is_human, vector<pair<int, int>>& player_order, int current_player_index, int players_amount, int& direction) {
    string rank = card.get_rank();
    if (rank == "A") {
...
int main() {
    ...
    for (int rd = 1; rd <= round; rd++) {
        ...
        while(remaining_players > 1) {
            ...
            cout << "Current player: Player " << player_order[current_player_index].first;
            if (player_order[current_player_index].first == place) cout << " (You)";
            cout << endl;
            
            int player_index = player_order[current_player_index].first - 1;
            Card chosen_card;
            ...
            if (legal_indices.empty()) {
                ...
                cout << "Player " << player_order[current_player_index].first;
                if (player_order[current_player_index].first == place) cout << " (You)";
                ...
                if (player_index + 1 == place) { // Player's turn
                    ...
                    cout << "Player " << player_order[current_player_index].first << " chose: " << chosen_card.get_rank() << " of " << chosen_card.get_suit() << endl;
                    data[player_index].getCards().erase(data[player_index].getCards().begin() + card_index);
                }

                // void apply_card_effect(const Card& card, int& pile, bool is_human, vector<pair<int, int>>& player_order, int current_player_index, int players_amount, int& direction)
                apply_card_effect(chosen_card, pile, player_index + 1 == place, player_order, current_player_index, players_amount, direction);
...
/*
    1. Changed the logic of the main part to solve the problem when the card is 4 and it's a 4-player game (requiring rotation)
*/