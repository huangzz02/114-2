...
int main() {
    ...
    for (int rd = 1; rd <= round; rd++) {
        ...
        // Step 0: Get the place of the game, and set the data
        ...
        while(remaining_players > 1) {
            for (int i = 0; i < remaining_players; i++) {
                ...
                if (player_index + 1 == place) { // Player's turn
                    ...
                    cout << "] (Choose 1~" << data[player_index].getCards().size() << "): ";

                    int choice;
                    cin >> choice;

                    int card_index = legal_indices[choice - 1];
    ...
    for (int i = 0; i < players_amount; i++) {
        string place_suffix = (i == 0) ? "st" : (i == 1) ? "nd" : (i == 2) ? "rd" : "th";
        cout << "Player " << data[i].getId() << ": " << data[i].getScore() << " (" << i + 1 << place_suffix << ")";
        if (data[i].getId() == place) cout << " (You)";
        cout << endl;
    }
    cout << endl;
